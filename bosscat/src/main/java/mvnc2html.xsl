...
<!--
    <xsl:template match="p[@level &lt; 0]">
        <xsl:element name="div">
            ...
            <xsl:apply-templates select="following-sibling::p[@level &gt; mylevel and preceding-sibling::p[@level &lt; 7][position() = 1] = current()]"/>
        </xsl:element>
    </xsl:template>
-->
